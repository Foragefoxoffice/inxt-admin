...
import { webinarsAPI } from '../../api/endpoints';
...
import { ArrowLeftOutlined, SaveOutlined, SearchOutlined, GlobalOutlined, PictureOutlined } from '@ant-design/icons';
...
const WebinarForm = () => {
  ...
  useEffect(() => {
    ...
    webinarsAPI.getOne(id)
      ...
      .catch(() => toast.error('Failed to load webinar'))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) { await webinarsAPI.update(id, payload); toast.success('Webinar updated'); }
      else { await webinarsAPI.create(payload); toast.success('Webinar created'); }
      navigate('/webinars');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to save webinar');
    } finally { setSaving(false); }
  };

  if (loading) return <LoadingSpinner text="Loading webinar..." />;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center gap-3 mb-6">
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/webinars')} />
        <div className="flex-1">
          <h2 className="text-lg font-bold text-slate-800 leading-none">{isEdit ? 'Edit Webinar' : 'New Webinar'}</h2>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-sm text-slate-500">{isEdit ? 'Update existing webinar' : 'Create a new webinar'}</span>
        ...
        <div className="flex gap-2">
          <Button onClick={() => navigate('/webinars')}>Cancel</Button>
      ...
      <div className="flex gap-6 items-start">
        <div className="flex-1 min-w-0 space-y-5">
          <Card className="!rounded-2xl !border-slate-100" title={<span className="font-bold text-slate-700">Webinar Info</span>}>
            <div className="space-y-4">
              <Form.Item label={<span className="font-medium text-slate-700">Title <span className="text-red-500">*</span></span>} className="!mb-0">
                <Input size="large" value={form.title} onChange={handleTitleChange} placeholder="Webinar title" className="!rounded-xl" />
              </Form.Item>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Form.Item label={<span className="text-sm font-medium text-slate-600">Category</span>} className="!mb-0">
                  <Input value={form.category} onChange={(e) => set('category', e.target.value)} placeholder="Technology, Business..." className="!rounded-xl !text-sm" />
                </Form.Item>
              </div>
            </div>
          </Card>
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-3">Webinar Content <span className="text-slate-400 font-normal">(Drag blocks to reorder)</span></p>
              ...
              <div className="space-y-4">
                <Form.Item label={<span className="text-sm font-medium text-slate-600">Meta Title</span>} className="!mb-0">
                  <div className="relative">
                    <Input value={form.seoMetaTitle} onChange={(e) => set('seoMetaTitle', e.target.value)} placeholder="Page title for search results" maxLength={70} className="!rounded-xl !pr-16" />
                ...
                <Form.Item label={<span className="text-sm font-medium text-slate-600">Keywords</span>} className="!mb-0">
                  <Input value={form.seoKeywords} onChange={(e) => set('seoKeywords', e.target.value)} placeholder="webinar, technology" className="!rounded-xl" />
                </Form.Item>
        ...
        <div className="w-[300px] flex-shrink-0 space-y-4 sticky top-6">
          <Card className="!rounded-2xl !border-slate-100" size="small" title={<span className="font-bold text-slate-700">Publish Settings</span>}>
            <div className="space-y-3">
              ...
              <div>
                ...
                <DatePicker value={form.publishDate} onChange={(date) => set('publishDate', date)} className="w-full !rounded-xl" format="DD/MM/YYYY" placeholder="Select date" />
              </div>
            </div>
          </Card>
          ...
          <Card className="!rounded-2xl !border-slate-100" size="small" title={<span className="font-bold text-slate-700">Author & Meta</span>}>
            <div className="space-y-3">
              ...
              <div>
                ...
                <Input value={form.tags} onChange={(e) => set('tags', e.target.value)} placeholder="ai, cloud, devops" className="!rounded-xl !text-sm" />
              </div>
            </div>
          </Card>
          ...
          <div className="flex gap-2">
            <Button block onClick={() => navigate('/webinars')} className="!rounded-xl">Cancel</Button>
