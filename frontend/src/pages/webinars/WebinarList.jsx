...
const WebinarList = () => {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await webinarsAPI.delete(deleteId);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Webinars</h2>
          <p className="text-slate-500 text-sm">{pagination?.total ?? 0} total sessions</p>
        </div>
        <Link to="/webinars/new" className="btn-primary flex items-center gap-2 text-sm">
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <EmptyState
            ...
            action={<Link to="/webinars/new" className="btn-primary text-sm">New Webinar</Link>}
          />
        ) : (
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-50">
                {webinars.map((webinar, idx) => (
                  <tr key={webinar._id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-6 py-4 text-sm text-slate-400 font-medium w-12">{(page - 1) * 10 + idx + 1}</td>
                    <td className="px-6 py-4">
                      <p className="text-sm font-semibold text-slate-800 max-w-xs truncate">{webinar.title}</p>
                      <p className="text-xs text-slate-400 mt-0.5 truncate">/{webinar.slug}</p>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">{webinar.category || '-'}</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center gap-1 text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">
                        {webinar.languageId?.code} — {webinar.languageId?.name}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => navigate(`/webinars/${webinar._id}/edit`)}
                        ...
                        <button
                          onClick={() => setDeleteId(webinar._id)}
