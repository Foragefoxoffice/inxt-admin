...
import {
  Form, Input, Select, DatePicker, Button, Card, Collapse,
  Tag, Alert
} from 'antd';
import {
...
const BlogForm = () => {
    ...
    blogsAPI.getOne(id)
      .then((res) => {
        ...
        setForm({
          ...
          author: b.author || '', tags: Array.isArray(b.tags) ? b.tags.join(', ') : '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) { await blogsAPI.update(id, payload); toast.success('Blog updated'); }
  ...
  if (loading) return <LoadingSpinner text="Loading blog..." />;
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-sm text-slate-500">{isEdit ? 'Update existing blog post' : 'Create a new blog post'}</span>
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          {/* Basic Info Card */}
          <Card className="!rounded-2xl !border-slate-100" title={<span className="font-bold text-slate-700">Basic Info</span>}>
          ...
          {/* Block Editor */}
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-3">Blog Content <span className="text-slate-400 font-normal">(Drag blocks to reorder)</span></p>
            <BlockEditor blocks={form.blocks} onChange={(blocks) => set('blocks', blocks)} />
          </div>

          {/* SEO Collapse */}
          <Collapse
            className="!rounded-2xl !border-slate-100 !bg-white"
            items={[{
              key: 'seo',
              label: (
              ...
              children: (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 gap-4">
                    <Form.Item label={<span className="text-sm font-medium text-slate-600">Meta Title <span className="text-slate-400 font-normal text-xs">(60 chars)</span></span>} className="!mb-0">
                    ...
                    <Form.Item label={<span className="text-sm font-medium text-slate-600">Meta Description <span className="text-slate-400 font-normal text-xs">(160 chars)</span></span>} className="!mb-0">
                      <div className="relative">
                        <TextArea value={form.seoMetaDescription} onChange={(e) => set('seoMetaDescription', e.target.value)}
                          placeholder="Brief description for search results" rows={2} maxLength={200} className="!rounded-xl !pr-16" />
                    ...
                    <Form.Item label={<span className="text-sm font-medium text-slate-600">Keywords <span className="text-slate-400 font-normal text-xs">(comma-separated)</span></span>} className="!mb-0">
                      <Input value={form.seoKeywords} onChange={(e) => set('seoKeywords', e.target.value)}
                        placeholder="cms, blog, react" className="!rounded-xl" />
                    </Form.Item>
                  </div>
                  <div className="border-t border-slate-100 pt-4">
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1">
                      <GlobalOutlined /> Open Graph / Social Share
                    </p>
                    <div className="grid grid-cols-2 gap-4">
                      <Form.Item label={<span className="text-sm font-medium text-slate-600">OG Title</span>} className="!mb-0">
                        <Input value={form.ogTitle} onChange={(e) => set('ogTitle', e.target.value)} placeholder="Defaults to post title" className="!rounded-xl" />
                      </Form.Item>
                      <Form.Item label={<span className="text-sm font-medium text-slate-600">OG Type</span>} className="!mb-0">
                        <Select value={form.ogType} onChange={(v) => set('ogType', v)} className="!w-full !rounded-xl">
        ...
        <div className="w-[300px] flex-shrink-0 space-y-4 sticky top-6">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <DatePicker
                  ...
                  onChange={(date) => set('publishDate', date)}
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={form.author} onChange={(e) => set('author', e.target.value)}
                  placeholder="Author name" className="!rounded-xl !text-sm" />
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wide">Tags <span className="font-normal text-slate-400 normal-case">(comma separated)</span></p>
                <Input value={form.tags} onChange={(e) => set('tags', e.target.value)}
                  placeholder="react, nodejs, cms" className="!rounded-xl !text-sm" />
              </div>
            </div>
          </Card>
          ...
          {/* Save Actions */}
          <div className="flex gap-2">
            <Button block onClick={() => navigate('/blogs')} className="!rounded-xl">Cancel</Button>
